...
/* [LC-TELE dev.203] tile-0 accessor: serializes the loss-control peak-hold snapshot (18 bytes,
 * see audio_pipeline.h LC_TELE_NUM_BYTES). Defined in audio_pipeline_t0.c (ON_TILE(0)); guarded so
 * tile-1 builds of this file don't reference the symbol. The dfu servicer task runs on tile 0. */
#if ON_TILE(0)
extern void audio_pipeline_get_lc_telemetry(uint8_t *buf);
#endif
...
control_ret_t dfu_servicer_read_cmd(control_resource_info_t *res_info, control_cmd_t cmd, uint8_t *payload, size_t payload_len)
{
    ...
    case DFU_CONTROLLER_SERVICER_RESID_GET_LC_STATE:
    {
        debug_printf("DFU_CONTROLLER_SERVICER_RESID_GET_LC_STATE\n");
        /* [LC-TELE dev.203] 18-byte loss-control telemetry; tile-0 accessor reads the audio-pipeline
         * peak-hold. payload already memset to 0 above, so on tile 1 (no accessor) it returns zeros. */
#if ON_TILE(0)
        audio_pipeline_get_lc_telemetry(payload);
#endif
        break;
    }
